...
def preprocess():
    ...
    #numpy.std(, axis=None, dtype=None, out=None, ddof=0, keepdims= < no value >)[source]

    deviation = np.std(train_data,axis=0)
    for i in deviation:
        if i > 1:
            valid_features = i
...
def nnObjFunction(params, *args):
    ...
    Z = np.column_stack(
        (sigmoid(np.dot(training_data, w1.T)), np.ones((sigmoid(np.dot(training_data, w1.T))).shape[0])))
    ...
    obj_val = (np.sum(-1 * (y * np.log(O) + (1 - y) * np.log(1 - O)))) / training_data.shape[0] + (
                (lambdaval / (2 * training_data.shape[0])) * (np.sum(np.square(w1)) + np.sum(np.square(w2))))
